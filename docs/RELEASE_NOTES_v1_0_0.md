...
**Binary size:** 0x95d60 bytes (~614 KB) — ~59% of 1 MB flash partition
**License:** MIT
...
- Deployed model: float32 weights (245 parameters, ~980 bytes in flash `.rodata`). The earlier optional int8 quantization path via `ml/quantize.py` + `firmware/components/tinyml_inference/model_data.cc` was removed in commit `295295c` since v1.0.0 has no TFLite Micro runtime to consume it (see REVIEW_FINDINGS B4).
- Embedded weights: `firmware/components/tinyml_inference/include/ml_weights.h` (regenerated by `ml/extract_weights.py` from `ml/models/saved_model`).
